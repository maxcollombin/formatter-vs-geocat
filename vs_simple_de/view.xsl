                    ...
                    <div class="row g-2">
                        <div class="col-12 col-md-4 d-flex justify-content-center justify-content-md-start">
                            <a class="btn btn-outline-danger btn-sm" 
                               href="{concat($urlBase, $identifier, '/formatters/vs_simple_de?width=_100&amp;mdpath=md.format.pdf&amp;output=pdf&amp;approved=true')}" 
                               target="_blank">Datenblatt drucken (PDF)</a>
                        </div>
                        <div class="col-12 col-md-4 text-center">
                            <a class="btn btn-outline-danger btn-sm" href="{concat($urlBase, $identifier, '/formatters/vs_full_de?language=ger')}">Komplettansicht</a>
                        </div>
                        <div class="col-12 col-md-4 d-flex justify-content-center justify-content-md-end">
                            <a class="btn btn-outline-danger btn-sm" 
                               href="{concat($urlBase, $identifier, '/formatters/vs_simple_fr?language=fre')}" 
                ...
                <main class="container">
                    <article class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <h3 class="mt-4">Informationen zur Aktualisierung</h3>
                            <div class="table-responsive">
                                <table class="table table-bordered table-sm table-striped table-hover align-middle table-50-50">
                                    <tbody>
                                        <xsl:if test="$statusCode">
                                            <tr><th scope="row">Bearbeitungsstatus</th><td><xsl:call-template name="codelist"><xsl:with-param name="code" select="$statusCode"/><xsl:with-param name="path">gmd:MD_ProgressCode</xsl:with-param></xsl:call-template></td></tr>
                                        </xsl:if>
                                        <xsl:if test="$creationDate">
                                            <tr><th scope="row">Erstellungsdatum</th><td><xsl:call-template name="formatDate"><xsl:with-param name="date" select="$creationDate"/></xsl:call-template></td></tr>
                                        </xsl:if>
                                        <xsl:if test="$revisionDate">
                                            <tr><th scope="row">Datum der Überarbeitung</th><td><xsl:call-template name="formatDate"><xsl:with-param name="date" select="$revisionDate"/></xsl:call-template></td></tr>
                                        </xsl:if>
                                        <xsl:if test="$contact">
                                            <tr>
