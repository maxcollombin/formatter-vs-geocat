...
<xsl:stylesheet version="1.0"
    ...
    <!-- Template pour les types de représentation spatiale (utilise codelists) -->
    ...
    <!-- Template pour les types d'objets géométriques (utilise codelists) -->
                    ...
                    <div class="row g-2">
                        <div class="col-12 col-md-4 d-flex justify-content-center justify-content-md-start">
                            <a class="btn btn-outline-danger btn-sm" 
                               href="{concat($urlBase, $identifier, '/formatters/vs_full_fr?width=_100&amp;mdpath=md.format.pdf&amp;output=pdf&amp;approved=true')}" 
                               target="_blank">Imprimer la fiche (PDF)</a>
                        </div>
                        <div class="col-12 col-md-4 text-center">
                            <a class="btn btn-outline-danger btn-sm" href="{concat($urlBase, $identifier, '/formatters/vs_simple_fr?language=fre')}">Vue simple</a>
                        </div>
                        <div class="col-12 col-md-4 d-flex justify-content-center justify-content-md-end">
                            <a class="btn btn-outline-danger btn-sm" 
                               href="{concat($urlBase, $identifier, '/formatters/vs_full_de?language=ger')}" 
                    ...
                    <article class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-sm table-striped table-hover align-middle table-50-50">
                                    <tbody>
                                        ...
                                        <xsl:if test="$basicGeodataID">
                                            <tr>
                                                <th scope="row">Identifiant géodonnée de base</th>
                                                <td><xsl:value-of select="$basicGeodataID"/></td>
                                            </tr>
                                        </xsl:if>
                                    </tbody>
                                </table>
                                <h3 class="mt-4">Information de l'identification</h3>
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-sm table-striped table-hover align-middle table-50-50">
                                            <tbody>
                                                ...
                                                <xsl:if test="$spatialRepresentationTypes">
                                                    <tr>
                                                        <th scope="row">Type de représentation spatiale</th>
                                                        <td>
                                                ...
                                                <xsl:if test="$geometricObjects">
                                                    <tr>
                                                        <th scope="row">Objets géométriques</th>
                                                        <td>
                                ...
                                <h3 class="mt-4">Mise à jour de la ressource</h3>
                                <div class="table-responsive">
                                    <table class="table table-bordered table-sm table-striped table-hover align-middle table-50-50">
                                        <tbody>
                                        ...
                                        <table class="table table-bordered table-sm table-striped table-hover align-middle table-50-50">
                                            <tbody>
                                                <xsl:for-each select="$onlineResources">
                                                    ...
                                                    <xsl:choose>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <xsl:for-each select="$allRestServices">
                                                                            <xsl:variable name="serviceURL" select="gmd:CI_OnlineResource/gmd:linkage/che:PT_FreeURL/che:URLGroup/che:LocalisedURL[@locale='#FR']"/>
                                                                            <xsl:variable name="serviceName" select="gmd:CI_OnlineResource/gmd:name/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR']"/>
                                                                            <xsl:variable name="serviceDescription" select="gmd:CI_OnlineResource/gmd:description/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR']"/>
                                                        ...
                                                        <xsl:when test="$resourceProtocol = 'WWW:DOWNLOAD:ZIP' and (contains($resourceName, 'GDB') or contains($resourceName, 'GPKG'))">
                                                            <!-- Aggrégation des différents liens -->
                                                            <xsl:if test="not(preceding-sibling::gmd:onLine[gmd:CI_OnlineResource/gmd:protocol/gco:CharacterString = 'WWW:DOWNLOAD:ZIP'][contains(gmd:CI_OnlineResource/gmd:name/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR'], 'GDB') or contains(gmd:CI_OnlineResource/gmd:name/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR'], 'GPKG')])">
                                                                ...
                                                                <tr>
                                                                    <th scope="row">Téléchargement des données (ZIP)</th>
                                                                    <td>
                                                                        <!-- Liens de téléchargement de la GDB -->
                                                                        <xsl:for-each select="$allZipResources[contains(gmd:CI_OnlineResource/gmd:name/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR'], 'GDB')]">
                                                                            <xsl:variable name="url" select="gmd:CI_OnlineResource/gmd:linkage/che:PT_FreeURL/che:URLGroup/che:LocalisedURL[@locale='#FR']"/>
                                                                            <a href="{$url}" target="_blank">ESRI File Geodatabase FileGDB (GDB)</a>
                                                                            <xsl:if test="position() != last()"><br/></xsl:if>
                                                                        </xsl:for-each>                                                                        
                                                                        <!-- Liens de téléchargement du GPKG -->
                                                                        <xsl:for-each select="$allZipResources[contains(gmd:CI_OnlineResource/gmd:name/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR'], 'GPKG')]">
                                                                            <xsl:variable name="url" select="gmd:CI_OnlineResource/gmd:linkage/che:PT_FreeURL/che:URLGroup/che:LocalisedURL[@locale='#FR']"/>
                                                                            <xsl:if test="position() = 1 and $allZipResources[contains(gmd:CI_OnlineResource/gmd:name/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR'], 'GDB')]"><br/></xsl:if>
                                                        ...
                                                        <xsl:when test="$resourceProtocol = 'WWW:DOWNLOAD-APP' and $resourceName = 'OpenData Valais'">
                                                            <!-- Aggrégation des différents liens -->
                                                            <xsl:if test="not(preceding-sibling::gmd:onLine[gmd:CI_OnlineResource/gmd:protocol/gco:CharacterString = 'WWW:DOWNLOAD-APP' and gmd:CI_OnlineResource/gmd:name/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR'] = 'OpenData Valais'])">
                                                                <xsl:variable name="allOpenDataServices" select="../gmd:onLine[gmd:CI_OnlineResource/gmd:protocol/gco:CharacterString = 'WWW:DOWNLOAD-APP' and gmd:CI_OnlineResource/gmd:name/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR'] = 'OpenData Valais']"/>
                                                                <tr>
                                                                    <th scope="row">Service<xsl:if test="count($allOpenDataServices) > 1">s</xsl:if> de téléchargement OpenData Valais</th>
                                                                    <td>
                                                                        <xsl:for-each select="$allOpenDataServices">
                                                                            <xsl:variable name="serviceURL" select="gmd:CI_OnlineResource/gmd:linkage/che:PT_FreeURL/che:URLGroup/che:LocalisedURL[@locale='#FR']"/>
                                                                            <xsl:variable name="serviceDescription" select="gmd:CI_OnlineResource/gmd:description/gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale='#FR']"/>
                                                                            <a href="{$serviceURL}" target="_blank">Lien</a>
                                                                            <xsl:if test="position() != last()"><br/></xsl:if>
                                                                        </xsl:for-each>
                                                                    </td>
                                                                </tr>
                                                       ...
                                                       <xsl:when test="contains($resourceName,'geodienste.ch')">
                                                            <tr>
                                                                <th scope="row">Lien de téléchargement geodienste.ch</th>
                                                                <td>
                                                                    <a href="{$resourceURL}" target="_blank">
                                                                        <xsl:choose>
                                                        ...
                                                        <xsl:when test="$resourceProtocol = 'CHTOPO:specialised-geoportal'">
                                                            <tr>
                                                                <th scope="row">Géoportail du Canton du Valais</th>
                                                                <td>
                                                ...
                                                <xsl:if test="$resourceFormats">
                                                    <tr>
                                                        <th scope="row">Format<xsl:if test="count($resourceFormats) > 1">s</xsl:if> de la ressource</th>
                                                        <td>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-sm table-striped table-hover align-middle table-50-50">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <th scope="row">Nom du standard de métadonnées</th>
